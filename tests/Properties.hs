...
prop_Lfoldr'_strict = expectFailure $ \ xs ys -> genericTake (L.length xs) (L.foldr' (:) [ ] (explosiveTail (xs `L.append` ys))) == L.unpack xs
...
prop_Dfoldr_lazy = \ xs -> genericTake (D.length xs) (D.foldr (:) [ ] (explosiveTailD xs)) == D.unpack xs
prop_Dfoldr'_strict = expectFailure $ \ xs ys -> genericTake (D.length xs) (D.foldr' (:) [ ] (explosiveTailD (xs `D.append` ys))) == D.unpack xs
